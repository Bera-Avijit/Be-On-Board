...
import { IconSend, IconBookmark, IconStar, IconTrendingUp, IconArrowUpRight, IconSparkles } from '@tabler/icons-react';
...
const CandidateDashboard = () => {
    ...
    // Assuming 'avijit@example.com' is the current user for demo purposes
    const myApps = apps.filter(app => app.email === 'avijit@example.com');

    const stats = [
        { label: 'Applications', value: myApps.length.toString(), icon: IconSend, color: 'bright-sun', increase: '+1' },
        { label: 'Saved Jobs', value: '15', icon: IconBookmark, color: 'blue', increase: '+3' },
        { label: 'Shortlisted', value: myApps.filter(a => a.status === 'SHORTLISTED').length.toString(), icon: IconStar, color: 'teal', increase: '+2' },
    ];
        ...
        <Container size="xl" className="py-16! min-h-screen!">
            ...
            <Box mb="5rem!">
                <Group justify="space-between" align="flex-end">
                    <Stack gap={0}>
                        <Group gap="sm">
                            <IconSparkles size={20} className="text-bright-sun-400" />
                            <Text size="xs" fw={900} className="text-mine-shaft-500 uppercase tracking-[0.4em]">Personal Growth</Text>
                        </Group>
                ...
                <Text size="md" className="text-mine-shaft-400! mt-4! font-medium! max-w-xl!">
                    Manage your career journey. Track your applications, view shortlisted roles, and stay updated with your progress.
                </Text>
            </Box>
